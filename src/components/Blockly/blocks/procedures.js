...
Blockly.Blocks['procedures_callnoreturn'] = {
    ...
    mutationToDom: function () {
        const container = document.createElement('mutation');
        container.setAttribute('test', this.getProcedureCall());
        for (let i = 0; i < this.arguments_.length; i++) {
            const parameter = document.createElement('arg');
            parameter.setAttribute('test', this.arguments_[i]);
    ...
    domToMutation: function (xmlElement) {
        ...
        const types = [];
        for (let i = 0, childNode; (childNode = xmlElement.childNodes[i]); i++) {
            console.log(childNode.getAttribute('type'));
            if (childNode.nodeName.toLowerCase() == 'arg') {
...
Blockly.Blocks['procedures_callreturn'] = {
    ...
    init: function () {
        ...
        this.setOutput(true, ''); // Start off as number
        this.setColour(getColour().procedures);
